...
import SocialLinks from "../utils/SocialLinks";
const Hero = ({ heroapi }) => {
  const { img, subtitle, title, videos, sociallinks, btntext } = heroapi;
  return (
    <>
        ...
        <div className="relative opacity-100 z-20 grid items-center justify-items-center nike-container">
          <div className="grid items-center justify-items-center mt-28 md:mt-24">
            ...
            <div className="grid items-center absolute top-[33vh] lg:top-[27vh] right-0 gap-3">
              {sociallinks?.map((val, i) => (
                <SocialLinks icon={val.icon} key={i} />
              ))}
            </div>
          </div>
          <div className="flex items-center">
            <img
              src={img}
              alt="hero-img/img"
              className="w-auto h-[38vh] lg:h-[30vh] md:h-[27vh] sm:h-[18vh] xsm:h-[16vh] transitions-theme -rotate-[25deg] hover:rotate-0 cursor-pointer object-fill"
            />
          </div>
        </div>
      </div>
